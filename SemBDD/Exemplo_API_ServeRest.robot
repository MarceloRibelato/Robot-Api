...
Cadastrar usuário de teste
    ...
    Log To Console   Resposta Retornada: ${\n}${RESPONSE.text}
    Log To Console   Resposta Retornada: ${\n}${BODY}
...
Listar o produto cadastrado
    ${PRECO}     Convert To Integer  155
    ${QUANT}     Convert To Integer  10
    ${HEADERS}   Create Dictionary  Authorization=${TOKEN}
    ...
    Dictionary Should Contain Item    ${RESPONSE.json()}    preco       ${PRECO}
    Dictionary Should Contain Item    ${RESPONSE.json()}    descricao   meu produto de teste
    Dictionary Should Contain Item    ${RESPONSE.json()}    quantidade  ${QUANT}
